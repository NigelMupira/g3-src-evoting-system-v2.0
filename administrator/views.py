...
def viewVotes(request):
    ...
    voted_voters_count = Voter.objects.filter(voted=1).count()
    ...
    for position in positions:
        ...
        for candidate in Candidate.objects.filter(position=position):
            ...
            votes = Votes.objects.filter(candidate=candidate).count()
            votes_count.append(votes)
            total_votes_count += votes  # Add to total votes
        chart_data[position] = {
            'candidates': list_of_candidates,
            'votes': votes_count,
            'pos_id': position.id
    ...
    context = {
        ...
        'total_votes_count': total_votes_count,  # New stat
        'page_title': "Real Time Votes"
    }
